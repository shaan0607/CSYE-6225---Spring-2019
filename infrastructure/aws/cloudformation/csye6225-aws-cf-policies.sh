...
createres=$(aws cloudformation create-stack  --stack-name $sn --capabilities CAPABILITY_NAMED_IAM --template-body file://csye6225-aws-cf-policies.json  --parameters ParameterKey=circleci,ParameterValue=$circleciuser ParameterKey=CDARN,ParameterValue=arn:aws:s3:::$CD_DOMAIN)
